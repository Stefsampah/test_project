...
puts "🚀 IMPLÉMENTATION DES NOUVELLES RÉCOMPENSES AVEC CONTENT_TYPE AUTORISÉS"
puts "=" * 80

require_relative 'config/environment'

# Configuration des nouveaux contenus en utilisant les content_type autorisés
new_rewards_data = {
  ...
  'backstage_videos' => [
    ...
    {
      content_type: 'backstage_video',
      reward_description: 'Backstage exclusif de Charles Doré en coulisses',
      badge_type: 'silver',
      quantity_required: 9,
...
puts "\n🔧 RÉSOLUTION DU PROBLÈME DE CONTRAINTE D'UNICITÉ"
puts "-" * 60
puts "✅ Utilisation de différents types de badges pour éviter les conflits"
puts "✅ Chaque récompense aura une combinaison unique : user_id + badge_type + quantity_required"
...
def create_reward_with_metadata(data, reward_type)
  begin
    ...
    puts "   ✅ Créée : #{data[:reward_description][0..50]}... (ID: #{reward.id})"
    puts "      🏷️ Badge: #{data[:badge_type]} | 🎯 Quantité: #{data[:quantity_required]}"
    return reward
...
puts "📈 Vous êtes passé de 23 à #{Reward.count} récompenses !"
puts "🔧 Problème de contrainte d'unicité résolu avec succès !"
