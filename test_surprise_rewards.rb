...
# Script de test pour le système de récompenses surprises
puts "🎁 Test du système de récompenses surprises"
puts "=" * 50
...
# 2. Tester le système de récompenses surprises
puts "\n🎁 Test du système de récompenses surprises :"

User.all.each do |user|
  puts "\n--- #{user.email} ---"
  
  # Vérifier les récompenses actuelles
  puts "📊 Récompenses surprises actuelles :"
  user.rewards.each do |reward|
    puts "  - #{reward.reward_type.humanize} : Récompense Surprise (#{reward.content_type})"
    puts "    Débloqué le : #{reward.unlocked_at&.strftime("%d/%m/%Y")}"
  end
  ...
  puts "\n🎯 Récompenses surprises éligibles :"
...
puts "\n🚀 Application du système de récompenses surprises..."
...
User.all.each do |user|
  ...
  if new_rewards.any?
    puts "  ✅ #{new_rewards.count} nouvelle(s) récompense(s) surprise(s) débloquée(s) :"
    new_rewards.each do |reward|
      puts "    - #{reward.reward_type.humanize} : Récompense Surprise (#{reward.content_type})"
    end
  else
    puts "  ⏳ Aucune nouvelle récompense surprise débloquée"
  end
end

puts "\n✅ Test du système de récompenses surprises terminé !"
puts "\n🎯 Résumé du système :"
puts "  • 3 badges = 1 récompense Challenge surprise (aléatoire)"
puts "  • 6 badges = 1 récompense Exclusif surprise (aléatoire)"
puts "  • 9 badges = 1 récompense Premium surprise (aléatoire)"
puts "  • Collection Arc-en-ciel = 1 récompense Ultime surprise (aléatoire)"
puts "  • Anti-répétition : jamais la même récompense 2 fois de suite"
puts "  • Image surprise : 🎁 pour toutes les récompenses"
puts "  • Mystère maintenu : contenu réel révélé uniquement lors de l'accès" 