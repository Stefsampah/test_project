#!/usr/bin/env ruby
# Script pour importer les données de jeu locales vers Heroku
# Usage: 
#   1. Copier les fichiers JSON sur Heroku: heroku run bash -a tubenplay-app
#   2. Créer tmp/ et copier les fichiers *_export_unique.json
#   3. Exécuter: heroku run rails runner import_to_heroku.rb -a tubenplay-app

puts "📥 Import des données de jeu vers Heroku..."

files_to_check = [
  'tmp/games_export_unique.json',
  'tmp/swipes_export_unique.json',
  'tmp/scores_export_unique.json',
  'tmp/user_badges_export_unique.json',
  'tmp/users_export.json'
]

missing_files = files_to_check.reject { |f| File.exist?(f) }
if missing_files.any?
  puts "❌ Fichiers manquants: #{missing_files.join(', ')}"
  puts "💡 Copiez d'abord les fichiers JSON sur Heroku"
  exit 1
end
...
# 1. Mettre à jour les utilisateurs (points, VIP, etc.)
...
games_data.each do |game_data|
  ...
  if user && playlist
    ...
    games_skipped += 1
    puts "  ⚠️  Partie ignorée: user=#{game_data['user_email']}, playlist=#{game_data['playlist_title']}" if games_skipped <= 5
  end
end
...
user_badges_data.each do |ub_data|
  ...
  if user && badge
    ...
    badges_skipped += 1
    puts "  ⚠️  Badge ignoré: user=#{ub_data['user_email']}, badge=#{ub_data['badge_name']}" if badges_skipped <= 5
  end
end
puts "✅ #{badges_imported} badges utilisateurs importés (#{badges_skipped} ignorés)"

# 5. Import des swipes (plus complexe car nécessite game_id)
puts "\n👆 Import des swipes..."
swipes_imported = 0
swipes_skipped = 0
swipes_data.each do |swipe_data|
  ...
  if user && playlist && video
    # Trouver ou créer le game correspondant
    ...
    swipes_skipped += 1
    puts "  ⚠️  Swipe ignoré: user=#{swipe_data['user_email']}, playlist=#{swipe_data['playlist_title']}, video=#{swipe_data['video_youtube_id']}" if swipes_skipped <= 5
  end
end
