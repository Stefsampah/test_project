#!/usr/bin/env ruby
# Script pour importer les données directement depuis stdin
# Usage: cat tmp/games_export_unique.json | heroku run "rails runner -e" -a tubenplay-app < import_to_heroku_direct.rb

require 'json'

puts "📥 Import des données de jeu vers Heroku..."

# Lire les données depuis stdin ou depuis les fichiers locaux
if ARGV.length > 0
  # Mode fichier local
  ...
  # Mode stdin (pour Heroku)
  puts "Lecture depuis stdin..."
  # Pour l'instant, on va utiliser les fichiers locaux
  # TODO: Implémenter la lecture depuis stdin
  exit 1
end
